<#
.SYNOPSIS
    Package the Sarifer using binaries from the signing directory.
.DESCRIPTION
    Builds the Sarifer NuGet packages with signed binaries from
...
function Copy-FromSigningDirectory {
    ...
    # Copy the Sarifer assemblies.
    # The names don't fit the pattern binary name == project name, so we copy them by hand.
    foreach ($framework in $Frameworks) {
        Copy-Item -Force -Path $SigningDirectory\$framework\2019\* -Destination $BinRoot\${Platform}_$Configuration\Sarif.Sarifer.VisualStudio\
        Copy-Item -Force -Path $SigningDirectory\$framework\2022\* -Destination $BinRoot\${Platform}_$Configuration\Sarif.Sarifer.VisualStudio.2022\
    }
}
