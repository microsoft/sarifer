<#
.SYNOPSIS
    Build, test, and package the VS Sarifer extension.
.DESCRIPTION
    Builds the VS Sarifer extension for multiple target frameworks, runs the tests, and creates
    NuGet packages.
...
$SolutionFile = "$SourceRoot\Sarifer.VisualStudio.sln"
$BuildTarget = "Rebuild"
...
function New-SigningDirectory {
    ...
    # Copy the Sarifer assemblies. The names don't fit the pattern binary name == project name,
    # so we copy them by hand.
    foreach ($framework in $Frameworks) {
        Copy-Item -Force -Path $BinRoot\${Platform}_$Configuration\Sarif.Sarifer\*Sarif.Sarifer*.dll -Destination $SigningDirectory\$framework\2019
        Copy-Item -Force -Path $BinRoot\${Platform}_$Configuration\Sarif.Sarifer.2022\*Sarif.Sarifer*.dll -Destination $SigningDirectory\$framework\2022
    }
}
